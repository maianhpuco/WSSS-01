...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run TPRO classification stage and generate pseudo labels"
    )
    parser.add_argument(
        "--config", required=True, help="Path to configs_maui/tpro_*.yaml"
    )
    ...
    base_cfg_source: Optional[object] = cls_model_cfg.get("config_file")
    if base_cfg_source is None:
        if "base_config" not in cls_model_cfg:
            raise KeyError(
                "Classification model config must define either 'config_file' or 'base_config'."
            )
    ...
    os.chdir(tpro_root)
    run_classification_inprocess(
        cls_config_output, cfg["runtime"]["classification"]
    )
    ...
    print("[TPRO] Pseudo-label generation complete.")
