...
PREDICTION_DIR = r"D:\NghienCuu\NghienCuuPaper\Source_Code\WSSS-01\src\modified_externals\ViLa-PIP\work_dirs\bcss_wsss\classification\predictions\2025-10-06-00-00\binary_mask"
OUTPUT_DIR = r"E:\Result_Binary_Mask"
...
def create_colored_binary_mask(cam, class_map, threshold=0.1):
    ...
    cam = cam.cpu().detach() if cam.is_cuda else cam
    if cam.ndim != 3 or cam.shape[0] != NUM_CLASSES:
        raise ValueError(f"Expected shape [4, height, width], got {cam.shape}")
    h, w = cam.shape[1], cam.shape[2]
    cam_softmax = F.softmax(cam, dim=0).numpy()  # Apply softmax directly on tensor
...
def visualize_original_and_binary_masks(model, x, batch_size, save_dir, feature_extractor, loss_function, fg_loss_fn, bg_loss_fn, h5_path=None, epoch=None, gt_mask=None, img_name=None, class_map=class_descriptions):
    ...
    for sample_idx in range(min(batch_size, 1)):
        ...
        if binary_mask_np.shape[0] == NUM_CLASSES:
        ...
        save_path = os.path.join(save_dir, f'original_binary_masks_{os.path.basename(os.path.splitext(img_name[0])[0])}_epoch_{epoch}.png') if epoch else os.path.join(save_dir, f'original_binary_masks_{os.path.basename(os.path.splitext(img_name[0])[0])}.png')
...
def visualize_original_and_binary_per_class(model, x, batch_size, save_dir, feature_extractor, loss_function, fg_loss_fn, bg_loss_fn, h5_path=None, epoch=None, gt_mask=None, img_name=None, class_map=class_descriptions):
    ...
    for sample_idx in range(min(batch_size, 1)):
        ...
        save_path = os.path.join(save_dir, f'original_binary_per_class_{os.path.basename(os.path.splitext(img_name[0])[0])}_epoch_{epoch}.png') if epoch else os.path.join(save_dir, f'original_binary_per_class_{os.path.basename(os.path.splitext(img_name[0])[0])}.png')
        plt.savefig(save_path, bbox_inches='tight', dpi=150)
        plt.close()
        print(f"Saved original and binary masks per class: {save_path}")

def infer_and_save_binary_masks(model, feature_extractor, loss_function, fg_loss_fn, bg_loss_fn, output_dir):
    ...
    model.eval()
    with torch.no_grad():
        for pth_file in tqdm(pth_files, desc="Generating Binary Masks", total=len(pth_files)):
            pth_path = os.path.join(PREDICTION_DIR, pth_file)
            # Extract base_name by removing 'binary_mask_' and everything after '_epoch_X'
            base_name = pth_file.replace('binary_mask_', '').split('_epoch_')[0]
            print(f"Processing {pth_file}, base_name: {base_name}")
            
            # Load binary mask from .pth file
            binary_mask = torch.load(pth_path, map_location=DEVICE)
            if binary_mask.ndim == 4 and binary_mask.shape[1] == NUM_CLASSES:
                binary_mask = binary_mask.squeeze(0)  # Remove batch dimension if present
            elif binary_mask.ndim == 3 and binary_mask.shape[0] == 1:
                binary_mask = binary_mask.squeeze(0)
            elif binary_mask.ndim != 3 or binary_mask.shape[0] != NUM_CLASSES:
                print(f"Unexpected binary_mask shape {binary_mask.shape} in {pth_file}, skipping.")
                continue

            # Move binary_mask to CPU for processing
            ...
            # Generate CAM (optional, can be skipped if not needed for inference)
            ...
            # Save colored binary mask
            colored_mask = create_colored_binary_mask(binary_mask, class_descriptions)  # Pass CPU tensor
            output_path = os.path.join(output_dir, f"binary_mask_{base_name}_epoch_{epoch}.png")
            colored_mask_pil = Image.fromarray(colored_mask)
            colored_mask_pil.save(output_path)
            print(f"Saved colored binary mask: {output_path}")

            # Copy .pth file to output directory
            torch.save(binary_mask.cpu(), os.path.join(output_dir, pth_file.replace(PREDICTION_DIR, '').lstrip('/\\')))
            ...
            visualize_original_and_binary_masks(model, x, 1, output_dir, feature_extractor, loss_function, fg_loss_fn, bg_loss_fn,
                                              h5_path=os.path.join(H5_TRAIN_DIR, f"{base_name}.h5"), epoch=epoch, gt_mask=gt_mask.unsqueeze(0), img_name=[img_name])
            visualize_original_and_binary_per_class(model, x, 1, output_dir, feature_extractor, loss_function, fg_loss_fn, bg_loss_fn,
                                                  h5_path=os.path.join(H5_TRAIN_DIR, f"{base_name}.h5"), epoch=epoch, gt_mask=gt_mask.unsqueeze(0), img_name=[img_name])
...
def main():
    ...
    infer_and_save_binary_masks(model, feature_extractor, loss_function, fg_loss_fn, bg_loss_fn, OUTPUT_DIR)
