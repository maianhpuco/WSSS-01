...
# Import PLIPProjector từ model/projector.py
import sys
import os
...
class MIL_fc(nn.Module):
    def __init__(self, gate=True, size_arg="small", dropout=False, n_classes=4, top_k=1):
        ...
        # Remove assert n_classes == 2 to allow flexibility
        # Changed in_dim from 768 to 1024
    ...
    def forward(self, h, return_features=False):
        ...
        if n_classes == 4:
            ...
            top_instance_idx = torch.topk(y_probs.max(
                dim=1)[0], self.top_k, dim=0)[1].view(1,)
...
class MIL_fc_mc(nn.Module):
    def __init__(self, gate=True, size_arg="small", dropout=False, n_classes=4, top_k=1):  # Default to 4 classes
        super(MIL_fc_mc, self).__init__()
        assert n_classes >= 2  # Allow 2 or more classes
        # Changed in_dim from 768 to 1024
        self.size_dict = {"small": [1024, 512]}
        size = self.size_dict[size_arg]
        fc = [nn.Linear(size[0], size[1]), nn.ReLU()]
