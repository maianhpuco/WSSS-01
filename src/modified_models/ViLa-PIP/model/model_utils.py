...
class Attn_Net_Gated(nn.Module):
    def __init__(self, L=512, D=256, dropout=False, n_classes=5, num_patches=197):
        r"""
        Attention Network with Sigmoid Gating (3 fc layers)

        args:
            L (int): input feature dimension per patch
        ...
        self.attention_a = [nn.Linear(L, D), nn.Tanh()]  # Input is L (feature dim), not num_patches
    ...
    def forward(self, x):
        # x shape: [batch_size, num_patches, L]
        print(f"Attn_Net_Gated input shape: {x.shape}")
        a = self.attention_a(x)  # [batch_size, num_patches, D]
        print(f"Attn_Net_Gated a shape: {a.shape}")
        b = self.attention_b(x)  # [batch_size, num_patches, D]
        print(f"Attn_Net_Gated b shape: {b.shape}")
        A = a.mul(b)  # [batch_size, num_patches, D]
        print(f"Attn_Net_Gated A before attention_c shape: {A.shape}")
        A = self.attention_c(A)  # [batch_size, num_patches, n_classes]
        print(f"Attn_Net_Gated A shape: {A.shape}")
        return A, x  # Return A and original x to preserve input structure
...
class MultiheadAttention(Module):
    ...
    Examples::
        >>> multihead_attn = nn.MultiheadAttention(embed_dim, num_heads, batch_first=True)  # Added batch_first=True
        >>> attn_output, attn_output_weights = multihead_attn(query, key, value)
    """
    bias_k: Optional[torch.Tensor]
    bias_v: Optional[torch.Tensor]

    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None, batch_first=True):  # Added batch_first=True
        ...
        self.batch_first = batch_first  # Added batch_first
        ...
        self.out_proj = Linear(embed_dim, embed_dim, bias=bias)  # Replaced _LinearWithBias with Linear
    ...
    def __setstate__(self, state):
        ...
        if 'batch_first' not in state:  # Ensure compatibility with older checkpoints
    ...
    def forward(self, query, key, value, key_padding_mask=None,
        ...
        Shape:
            ...
            - query: :math:`(N, L, E)` where N is the batch size, L is the target sequence length, E is
              the embedding dimension (when batch_first=True).
            ...
            - attn_output: :math:`(N, L, E)` where N is the batch size, L is the target sequence length,
              E is the embedding dimension (when batch_first=True).
            - attn_output_weights: :math:`(N, L, S)` where N is the batch size,
              L is the target sequence length, S is the source sequence length.
        """
        if self.batch_first:  # Adjust for batch_first=True
        ...
        if self.batch_first:  # Transpose back to batch_first format
...
def pt2graph(coords, features, threshold=5000, radius=9):
    ...
    coords, features = np.array(
        coords.cpu().detach()), np.array(features.cpu().detach())
...
def pairwise_distances(x):
    bn = x.size(0)
    x = x.reshape(bn, -1)  # Use reshape instead of view
...
def calculate_MI(x, y, s_x=1.0, s_y=1.0):
